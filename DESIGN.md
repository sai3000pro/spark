...
2. **The hero** (v5.6, `components/hero/JournalHero.tsx`) — the journal's night
   ...
   then a PAINTERLY forest in planes (v5.7): every tree is painted by a
   generative brush script (hundreds of directional daubs over a solid dark
   undermass, light concentrated on two or three lit lobes, blue-grey forked
   trunks with one lit edge, a few daubs blowing off the lee side — the
   painterly-reference technique in the journal's night values), exported as
   sprites to `public/hero/journal/` (`tree-a/b/c.webp`, a blurred `grove.webp`
   band, a tufted `grass.webp` foreground). Far grove → fog → mid trees → fog →
   two near tree portraits framing the copy → painted grass. Brass fireflies
